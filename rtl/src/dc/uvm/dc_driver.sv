class dc_driver extends uvm_driver #(dc_program #(MIN_HOLD_CYCLES));

    virtual dc_input_if vif;

    function new(string name = "dc_driver", uvm_component parent = null);
        super.new(name, parent);
        `uvm_info("dc_driver", "new is called", UVM_LOW);
    endfunction


    virtual function void build_phase(uvm_phase phase);

        super.build_phase(phase);
        `uvm_info("dc_driver", "build_phase is called", UVM_LOW);

        if (!uvm_config_db#(virtual dc_input_if)::get(this, "", "vif", vif))
            `uvm_fatal("dc_driver", "virtual interface must be set");

    endfunction


    typedef struct {
        logic [DC_INSN_WIDTH] pcmem_q [$];
        logic [DC_INSN_WIDTH] imem_q [$];
    } mem_content_t;


    virtual function mem_content_t get_mem_content(dc_program pgm);

        mem_content_t m;
        int insn2addr_map [logic [DC_INSN_WIDTH-1:0]];
        logic [DC_INSN_WIDTH-1:0] bits;
        int addr = 0;

        foreach (pgm.insns[i]) begin

            bits = pgm.insns[i];

            if (insn2addr_map.exists(bits)) begin
                m.pcmem_q.push_back(insn2addr_map[bits]);
            end
            else begin
                insn2addr_map[bits] = addr;
                m.imem_q.push_back(bits);
                m.pcmem_q.push_back(addr);
                addr++;
            end

        end

        return m;

    endfunction


    virtual task burst_ctrl(dc_ctrl_t ctrl);
        vif.i_ctrl_regs[0] <= 32'(ctrl.w_dvsr);
        vif.i_ctrl_regs[1] <= 32'(ctrl.w_delay_cycles);
        vif.i_ctrl_regs[2] <= 32'(ctrl.w_cs_up_cycles);
        vif.i_ctrl_regs[3] <= 32'(ctrl.w_ldac_cycles);
        vif.i_ctrl_regs[4] <= 32'h0;
        @(posedge vif.i_clk);
        vif.i_ctrl_regs[4] <= 32'h1;
        @(posedge vif.i_clk);
        vif.i_ctrl_regs[4] <= 32'h0;
        @(posedge vif.i_clk);
    endtask


    virtual task burst_mem(mem_content_t m, int unsigned iters);
        logic [DC_INSN_WIDTH-1:0] insn;

        // write unique instructions to IMEM
        foreach (m.imem_q[i]) begin
            insn = m.imem_q[i];
            vif.i_seq_regs[3] <= 32'(i);  // IST_ADDR_REG
            for (int r = 0; r < DC_REG_PER_INSN; r++)
                vif.i_seq_regs[4 + r] <= 32'(insn >> ((DC_REG_PER_INSN - 1 - r) * 32));
            @(posedge vif.i_clk);
            vif.i_seq_regs[4 + DC_REG_PER_INSN] <= 32'h1;  // IST_STRB_REG
            @(posedge vif.i_clk);
            vif.i_seq_regs[4 + DC_REG_PER_INSN] <= 32'h0;
            @(posedge vif.i_clk);
        end

        // write PC sequence to PCMEM
        foreach (m.pcmem_q[i]) begin
            vif.i_seq_regs[0] <= 32'(i);               // PCST_ADDR_REG
            vif.i_seq_regs[1] <= 32'(m.pcmem_q[i]);   // PCST_REG
            @(posedge vif.i_clk);
            vif.i_seq_regs[2] <= 32'h1;                // PCST_STRB_REG
            @(posedge vif.i_clk);
            vif.i_seq_regs[2] <= 32'h0;
            @(posedge vif.i_clk);
        end

        // set depth (inclusive last address) and iters
        vif.i_seq_regs[5 + DC_REG_PER_INSN] <= 32'(iters);                    // ITERS_REG
        vif.i_seq_regs[6 + DC_REG_PER_INSN] <= 32'(m.pcmem_q.size() - 1);    // DEPTH_REG
        @(posedge vif.i_clk);
    endtask


    virtual task run_phase(uvm_phase phase);
        mem_content_t m;
        super.run_phase(phase);

        vif.i_seq_regs = '0;
        vif.i_ctrl_regs = '0;
        while (vif.i_rst)
            @(posedge vif.i_clk);

        forever begin
            seq_item_port.get_next_item(req);
            if (req.has_ctrl)
                burst_ctrl(req.ctrl);
            m = get_mem_content(req);
            burst_mem(m, req.iters);
            seq_item_port.item_done();
        end
    endtask

endclass
