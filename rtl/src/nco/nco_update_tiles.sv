...
module nco_update_tiles
   #(parameter NUM_DAC_TILE=4,
     ...
     parameter NCO_STATUS_REGS=1)
    (input  logic i_clk, i_rst,

     // nco update registers dac tile 228/229/230/231
     input  logic [0:NUM_DAC_TILE-1][0:NCO_PER_DAC_TILE*3][31:0] i_dac_nco_ctrl_regs,
     output logic [0:NUM_DAC_TILE-1][0:NCO_STATUS_REGS-1][31:0] o_dac_nco_status_regs,
     ...
     input  logic [0:NUM_ADC_TILE-1][0:NCO_PER_ADC_TILE*3][31:0] i_adc_nco_ctrl_regs,
     output logic [0:NUM_ADC_TILE-1][0:NCO_STATUS_REGS-1][31:0] o_adc_nco_status_regs,
    ...
    ************/
    ...
    for (genvar i = 0; i < NUM_DAC_TILE; i++) begin : DAC_NCO_UPDATE_GEN
        nco_update #(
            ...
            .NUM_STATUS_REGS(NCO_STATUS_REGS)
        ) DAC_NCO_UPDATE (
            .i_clk(i_clk),
            .i_rst(i_rst),

            .i_ctrl_regs(i_dac_nco_ctrl_regs[i]),
            .o_status_regs(o_dac_nco_status_regs[i]),
    ...
    for (genvar i = 0; i < NUM_ADC_TILE; i++) begin : ADC_NCO_UPDATE_GEN
        nco_update #(
            ...
            .i_ctrl_regs(i_adc_nco_ctrl_regs[i]),
            .o_status_regs(o_adc_nco_status_regs[i]),
